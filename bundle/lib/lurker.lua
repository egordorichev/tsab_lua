...
--
-- Patched for tsab by @egordorichev in September 2018
---
...
local dir = tsab.fs.get_directory_files
local isdir = tsab.fs.is_directory
local time = tsab.getTime or os.time
local lastmodified = tsab.fs.get_last_modified
...
function lurker.initwrappers()
  for _, v in pairs(lovecallbacknames) do
    lurker.funcwrappers[v] = function(...)
      local args = {...}
      xpcall(function()
        return lurker.lovefuncs[v] and lurker.lovefuncs[v](table.unpack(args))
      end, lurker.onerror)
    end
    lurker.lovefuncs[v] = tsab[v]
  end
...
function lurker.updatewrappers()
  for _, v in pairs(lovecallbacknames) do
    if tsab[v] ~= lurker.funcwrappers[v] then
      lurker.lovefuncs[v] = tsab[v]
      tsab[v] = lurker.funcwrappers[v]
    end
  end
end
